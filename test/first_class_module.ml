...
下面是  声明 module 类型 和 定义 module 的语法
...
let int_bumper = (module Int_bumper : Bumpable);;   (* 转换成 第一类 模块 *)
...
***************************************************************************************************************************************************
***************************************************************************************************************************************************
以下函数采用两个参数： Bumpable 模块 和与  模块的 t 类型相同类型的元素列表：  
...
(* 
  该函数的类型签名为：
      ...
      所以  (type a) 的写法就是其他语言中的 泛型参数   T 的意思
*)
