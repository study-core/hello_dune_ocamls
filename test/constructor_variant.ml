...
######################################################################################################################################################
  ...
  变体类型的声明列出了该类型值的所有可能形式.
  每个案例都由一个称为构造函数的名称来标识, 它既用于构造变体类型的值,又用于通过模式匹配检查它们.
  构造函数名称大写, 以区别于变量名称 (必须以小写字母开头).
...
let hoge = function
  ...
  | `Piyo -> "piyo";;
(* 
  因为这里没写 | _ -> 所以类型推断出是  下限 [< `Fuga | `Hoge | `Piyo ] 
  如果写了 | _ -> 那么类型推断出是  上限 [> `Fuga | `Hoge | `Piyo ] 
*)

(* 
######################################################################################################################################################
[>] 泛型限定             表示接受 >= 这些类型的 超集类型的 值
######################################################################################################################################################   
*)
...
type type_1 = [`Gavin | `Xu];;

type type_2 = [`Gavin | `Xu | `Jiacan];;
