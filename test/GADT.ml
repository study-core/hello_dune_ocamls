...
(* 这是一个 ADT 类型，它是一个标准的列表类型，它是一个泛型类型，它的类型参数是 'a , 这里的 'a 必须是一致的，不能是不同的类型 *)
type 'a standard_list =
  | Nil                  (* 空列表，它的类型标签是 'a standard_list *)
  | Cons of 'a * 'a standard_list (* 非空列表，它的类型标签是 'a standard_list *)

如 

Nil 的类型是 int standard_list
Cons (10, Nil) 的类型也是 int standard_list

如果你尝试将一个 int standard_list 和一个 string standard_list 混合起来，编译器会阻止你，因为 'a 必须统一


而 

(* 这是一个 GADT 类型，它是一个表达式类型，它是一个泛型类型, _ 只是个类型占位符而已，可以是不同的类型 *)
type _ expr =
  | Int  : int expr
  | Bool : bool expr


Int 返回的“形状”是 int expr。Bool 返回的“形状”是 bool expr。这两个返回类型看起来是独立的



(* 如果你在这里写 'a：
       错误！标准的 ADT 语法不能有类型注解 

  编译器会报错，因为它期待 Int 和 Bool 都返回 'a expr，
  而你却告诉它一个返回 int expr，一个返回 bool expr，这导致了矛盾     
*)
type 'a expr =
  | Int : int expr
  | Bool : bool expr
...
标准的变体无法根据其内部携带的数据类型来改变其自身的类型 

标准的 ADTs 就像你买到的一个普通的乐高套装。这个套装里的所有零件都属于同一个大类——它们都是“乐高积木件”。
无论里面的积木是红色的小方块、蓝色的长条、还是透明的圆片，它们共同的类型都是 积木件。

(* type item 是所有情况的统一类型 *)
type item =
  | IntItem of int       (* 红色小方块，里面放数字 *)
  | StringItem of string (* 蓝色长条，里面放字符串 *)
  | BoolItem of bool     (* 透明圆片，里面放布尔值 *)
;;  

你无法定义一个规则，要求一个特定的容器只能装红色的方块。
你只能说：“这个容器里装的是 item”。当你从容器里拿东西时，你需要再次检查它的颜色：


let process item =
  match item with
  | IntItem i -> (* 好的，我知道这是红色的了 *) i + 1
  | StringItem s -> (* 好的，我知道这是蓝色的了 *) s ^ "!"
  | BoolItem b -> (* 好的，我知道这是透明的了 *) not b
;;

而 GADTs 就像你买到的一个特殊的乐高套装。这个套装里的所有零件都属于同一个大类——它们都是“乐高积木件”。
红色方块的类型是 红色积木；蓝色长条的类型是 蓝色积木。它们不再共享一个普通的 积木件 类型。

(* type _ expr 是一个 GADT *)
type _ expr =
  | Int   : int expr    (* 红色方块，它的类型标签是 int expr *)
  | Bool  : bool expr   (* 蓝色长条，它的类型标签是 bool expr *)
  | Add   : int expr * int expr -> int expr  (* 组装规则：只能用两个红色方块组合出另一个红色方块, 它的类型标签是 int expr*)
;;


Int : int expr：Int 构造器返回的类型是 int expr, 它的类型标签是 int expr。
Add : int expr * int expr -> int expr：Add 期望两个 int expr 作为输入，结果也是 int expr, 它的类型标签是 int expr。


let valid = Add (Int, Int) (* ✅ 成功，标签匹配 *)

(* let invalid = Add (Int, Bool) *) (* ❌ 编译错误！标签不匹配！ *)
