...
查看 first_class_module.ml 文件 讲解
######################################################################################################################################################
...
*******************************************  
叫做  【破坏性替换】    <Module_type> with type <type> := <type'>
*******************************************



|  特性	     |   普通共享限制 (with type t = int)	|   破坏性替换 (with type t := int)       |
|  ------------------ | ------------------ | ------------------ | 
|  逻辑含义	 |  等价声明 (Equality)	               |   彻底替换 (Substitution)             |
|  可见性	   |  模块中依然存在 type t 这个名字     |   t 会从模块接口中彻底消失               |
|  内部引用	 |  外部依然可以写 M.t                |   外部不能再写 M.t，必须直接用 int        |  
|  逻辑效果	 |  t 变成了 int 的一个别名           |	  接口中所有出现 t 的地方都直接换成了 int  |
|  外部引用	 |  外部可以调用 M.xxx	              |   外部不能调用 M.xxx                       |
|  签名复杂度	 |  签名长度不变（多了一个别名定义）	|   签名变短（少了一个类型定义）           |
...
现在没有 endpoint 类型：
            ...
            它的所有出现都已被 int 替换。与共享约束一样，我们也可以在函子的上下文中使用它：
