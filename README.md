...
1. dune-project 文件，包含项目范围的配置数据。这是一个非常小的：(项目级 配置)

```
(lang dune 3.4)
```

2. dune 文件，其中包含实际的【构建指令】。一个项目可能有多个，具体取决于源的组织。 (目录级 配置)
...
除了性能之外，两个编译器生成的可执行文件具有几乎相同的行为。有几件事需要注意。首先，字节码编译器 
 ocamlc 可以在更多的架构上使用，并且拥有一些本机代码所没有的工具。编译时间上字节码编译器也比本机代码编译器更快。
